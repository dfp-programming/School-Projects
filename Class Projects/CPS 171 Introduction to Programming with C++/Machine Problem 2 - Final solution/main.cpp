...
in a another text file called mp2accept.txt. The information for each applicant will say if they were accepted or not, and the reason for rejection. 
...
Only the first five applicants can get in even if their parents are alumni. To get into Music, a minimum individual verbal and math SAT of 500 is required. Only the first three applicants can get in. 
...
int main() {
	...
	ifstream readFromFile("mp2accept.txt");
	ofstream writeToFile("accepted_candidates.txt");
	...
	while(readFromFile >> school >> gpa >> mathSat >> verbalSat >> alumnus){
		...
		// The next set of  statements will print to file messsages with the applicant's info that didn't get into school, and will state why. 
			...
			//This part is for applicants rejected from music. They either have verbal and math SAT less than 500 or the school was full. 
			else if(school=='M'){
				writeToFile <<  "Applying to Music" << endl;
